...
- `GITLAB_API_TOKEN` — GitLab 个人访问令牌（Personal Access Token），在 `https://www.lejuhub.com/-/profile/personal_access_tokens` 创建，需勾选 `api` 权限
- `GITLAB_API_URL` — GitLab 实例地址，固定为 `https://www.lejuhub.com`

#### 变量读取方式

`GITLAB_API_TOKEN` 存储在系统用户环境变量中。**首次执行任何 API 调用前**，必须用以下命令检查 Token 是否已配置：
...
如果返回空，提示用户去 `https://www.lejuhub.com/-/profile/personal_access_tokens` 创建 Token（勾选 `api` 权限），然后设置为用户环境变量 `GITLAB_API_TOKEN`。

#### API 调用中的变量引用

由于 Windows PowerShell 不支持 `${VAR}` 形式的 shell 变量展开，在所有 `curl` 命令中，**先通过当前平台的读取方式获取 Token 存入局部变量，再在 curl 命令中引用**：

**Windows (PowerShell) 写法：**

```powershell
$token = [System.Environment]::GetEnvironmentVariable("GITLAB_API_TOKEN", "User")
$apiUrl = "https://www.lejuhub.com/api/v4"
$projectId = "pc%2Fkuavo-desktop"
curl -s --header "PRIVATE-TOKEN: $token" "$apiUrl/projects/$projectId/issues"
```

**Linux/macOS (bash) 写法：**

```bash
API_URL="https://www.lejuhub.com/api/v4"
...
```
https://www.lejuhub.com/api/v4
```

所有 curl 命令需携带认证头：`--header "PRIVATE-TOKEN: <token>"`，Token 统一通过上述平台对应的读取方式获取。
...
以下命令中，`$token` 已在执行前通过上述平台对应方式读取，`$projectId` 已通过项目自动识别方式获取，**切勿重复获取**。
...
**执行创建分支前，必须先调用 `skill` 工具加载 `git-conventions` 技能**，严格按照其分支命名规范（`userSlug/type/snake_keywords`）确定分支名，并完成重复检查后，再执行以下 API 调用：

```bash
curl -s --header "PRIVATE-TOKEN: $token" \
  --data-urlencode "branch=<按 git-conventions 规范生成的分支名>" \
...
**执行创建 MR 前，必须先调用 `skill` 工具加载 `git-conventions` 技能**，严格按照其 MR 描述规范生成描述内容后，再执行以下 API 调用：
...
以下命令中，`$token` 已在执行前通过上述平台对应方式读取，**切勿重复获取**。

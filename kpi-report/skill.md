...
从系统环境变量 `GITLAB_API_TOKEN` 和 `GITLAB_API_URL` 中读取。

### 3. 获取用户 ID

```bash
curl -s -H "PRIVATE-TOKEN: {TOKEN}" "{API_URL}/api/v4/users?username=lifangzheng"
```

从返回 JSON 中提取 `id` 字段（当前为 1983）。

### 4. 获取活动记录

```bash
curl -s -H "PRIVATE-TOKEN: {TOKEN}" "{API_URL}/api/v4/users/{id}/events?after={start_date}&before={end_date}&per_page=100"
```
